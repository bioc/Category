...
%\VignetteDepends{Biobase, genefilter, hgu95av2, RColorBrewer}
...
##filter on IQR, but use fast new methods
lowQ = rowQ(esetA, floor(.25*79))
...
pathways. We will make the decision that we are not interested in
pathways that have fewer than 5 members that are in the experimentally
observed genes.

<<Amat>>=
##compute the Amat, and rearrange it to follow the order of genes
Amat = t(PWAmat("hgu95av2"))
AmER = Amat[,names(fL)]
rs = rowSums(AmER)

##FIXME: you need to decide how big a pathway you care about...
AmER2 = AmER[rs>5,]
rs2 = rs[rs>5]
...
activity among those with BCR/ABL none of the stand out the way that
the ribosome pathway does. We can find that pathway and examine the
data a bit more. Then in the next section we make use of the
permutation approach to assess significance.
...
\begin{figure}[tp]
  \centering
<<KEGGmnplot, fig=TRUE, include=FALSE, echo=FALSE, results=hide>>=
...
<<setupHMcols, results=hide, echo=FALSE>>=
...
\begin{figure}[tp]
  \centering
<<Ribosomeheatmap, fig=TRUE, include=FALSE, echo=FALSE, results=hide>>=
...
\begin{figure}[tp]
  \centering
<<HMbysex, fig=TRUE, include=FALSE, echo=FALSE, results=hide>>=
...
<<setupHC, echo=FALSE, results=hide>>=
lnhC = length(highC)
@
...
\begin{figure}[tp]
  \centering
<<mnplot1, fig=TRUE, width=5, height=5, include=FALSE, echo=FALSE, results=hide>>=
...
\begin{figure}[tp]
  \centering
<<mnplot2, fig=TRUE, include=FALSE, echo=FALSE, results=hide>>=
...
\begin{figure}[tp]
  \centering
<<mnplot3, fig=TRUE, include=FALSE, echo=FALSE, results=hide>>=
...
\begin{figure}[tp]
  \centering
<<HMperm, fig=TRUE, include=FALSE, echo=FALSE, results=hide>>=
...
virtually any other per-category function is quite simple and the cade
to do this is available as the \Rfunction{applyByCategory} function.
